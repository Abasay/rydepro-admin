...
const CustomDatePicker: React.FC<Props> = ({
  ...
  const handleDateChange = (date: Date | null) => {
    ...
    switch (active) {
      case 'from':
        setFrom(format(date as Date, 'do MMM yyyy'));
        break;
      case 'to':
        setTo(format(date as Date, 'do MMM yyyy'));
        break;
      case 'date':
        setDate(format(date as Date, 'do MMM yyyy'));
        break;

      case 'dob':
        if (setDOB) setDOB(format(date as Date, 'do MMM yyyy'));
        // setDate(format(date as Date, 'do MMM yyyy'));
        break;
      default:
        break;
    }
    setIsPickerOpen(false); // Close picker after selecting a date
  };
  ...
  return (
    <div className=''>
      ...
      {isPickerOpen && (
        <div className='absolute  w-full top-[102px] -left-2 z-[9999]'>
          <DatePicker
            selected={selectedDate}
            onChange={handleDateChange}
            inline
            className='bg-white w-full shadow-lg p-2 rounded-md'
          />
        </div>
      )}
