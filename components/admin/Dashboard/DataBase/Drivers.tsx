...
const Drivers = () => {
  const { activeHeader, filters, setFilters, showSingleDriver } = useDB();
  ...
  useEffect(() => {
    if (activeHeader === 'All Drivers') {
      ...
      const newChauffeurs = drivers.filter(
        (driver) => (driver['Account Type'] as string).toLowerCase() === 'Chauffeur Drivers'.toLowerCase()
      );
      setAllDrivers(newChauffeurs);
    } else {
      const newDrivers = drivers.filter(
        (driver) => (driver['Account Type'] as string).toLowerCase() === activeHeader.toLowerCase()
      );
      setAllDrivers(newDrivers);
    }
  }, [activeHeader]);
  ...
  return (
    <div className=" mt-6 flex flex-col gap-4 px-6 min-h-[300px] max-h-[771px] pb-10">
      ...
      <div className=" mb-10 w-full bg-[#FFFFFF] rounded-2xl pt-4 max-w-full overflow-auto min-h-min max-h-[989px]">
          ...
          <>
            <div className=" flex justify-between items-center pr-4 overflow-auto">
              ...
              <div className=" flex gap-6 border-b-2 min-w-[1100px] border-[#8A8A8A]  items-center">
                {['All', 'Approved Drivers', 'Pending Drivers', 'Rejected Drivers', 'Suspended Drivers'].map(
                  (header, idx) => (
                    <div key={idx} className=" flex flex-col gap-3">
                      <div className=" px-5 items-center flex gap-4">
                        <span
                          className={`${header === 'All' ? 'text-[#0E0E0E] font-medium' : 'text-[#8A8A8A]'} w-full`}
                        >
                          {header}
                        </span>
        ...
        <div className=" w-full pr-10 h-full pb-32 max-h-[calc(100vh - 150px)]  overflow-auto">
          {['All Drivers', 'Livery Company'].includes(activeHeader) && <DriversTable allDrivers={allDrivers} />}
