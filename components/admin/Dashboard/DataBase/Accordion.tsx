...
const Accordion: React.FC<AccordionProps> = ({ items, filters, setFilters }) => {
  ...
  const handleInputs = (e: React.FormEvent, subItem: { name: string }, item: { title: string }, idx: number) => {
    ...
    const updateFilters = (title: string, newFilters: string[], subItemKey: string) => {
      setFilters((prev: any) => ({
        ...prev,
        [title]: newFilters,
    ...
    if (!isSameName) {
      ...
      if (!filters[item.title]) {
        ...
        if (filters[item.title].includes(filters[subItem.name])) {
          ...
          updateFilters(
            item.title,
            [filters[subItem.name], ...filters[item.title].filter((f: string) => f !== filters[subItem.name])],
            subItem.name
          );
        }
      }
    ...
    if (filters[subItem.name]) {
      if (!filters[item.title]) {
        ...
        if (filters[item.title].includes(filters[uniqueKey][0])) {
          ...
          updateFilters(item.title, [...filters[item.title], filters[uniqueKey][0]], uniqueKey);
        }
      }
    } else {
      ...
      if (!filters[item.title]) {
        ...
        if (filters[item.title].includes(filters[uniqueKey])) {
          ...
          updateFilters(item.title, [...filters[item.title], filters[uniqueKey]], uniqueKey);
        }
      }
    }
  };
  ...
  return (
    <div className="flex flex-col justify-center gap-1">
      {items.map((item, index) => (
        <div key={index} className="rounded-[8px] flex flex-col gap-3 bg-[#F8F8F8] p-2">
          <button
            ...
            <div className="flex flex-col gap-4">
              {item.subTitle.map((subItem: any, idx) => {
                return (
                  <>
                    {subItem?.headerTitle && (
                      <span className=" text-sm font-medium text-[#0E0E0E]">{subItem.headerTitle}</span>
                    )}
                    {subItem.isSelect && (
                      <div key={`${subItem}_idx`} className=" flex flex-col gap-1">
                        <div
                              ...
                              setFilters((prev: any) => {
                                return {
                                  ...
                                  [item.title]: filters[item.title] && [
                                    ...filters[item.title].filter((filterItem: string) => filterItem !== subItem.name),
                                  ],
                                };
                              });
                            } else {
                        ...
                        >
                          {filters[item.title]?.includes(subItem.name) ? (
                            ...
                            >
                              <rect width="16" height="16" rx="2.66667" fill="#0E0E0E" />
                              <path
                          ...
                          <span className="text-[#0E0E0E] text-sm">{subItem.name}</span>
                        </div>

                        {subItem.hasInput && subItem.isSelect && filters[item.title]?.includes(subItem.name) && (
                          <form
                            action=""
                                    ...
                                    return {
                                      ...
                                      [item.title]: [
                                        ...filters[item.title].filter(
                                          (filterItem: string) => filterItem !== subItem.name
                                        ),
                        ...
                        {filters[item.title]?.includes(subItem.name) && subItem.hasSelect && (
                          <>
                            {subItem.isYesOrNo && (
                              <select
                                name={subItem.name}
                    ...
                    {subItem.hasInput && !subItem.isSelect && (
                      <div className=" flex gap-1 flex-col items-start">
                        {subItem.name !== item.title && <span className="text-[#0E0E0E] text-sm">{subItem.name}</span>}
                        {
                          <form
                            action=""
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  setFilters((prev: any) => {
                                    return {
                                      ...prev,
                                      [`${subItem.name}_${idx}`]: [e.target.value],
                                    };
                                  });
                                  console.log(filters);
                                }}
                    ...
                    {subItem.hasSelect && !subItem.hasInput && !subItem.isSelect && (
                      <div className=" flex gap-1 flex-col items-start">
                        {subItem.name.toLowerCase().includes('language') && <span>Select a Language</span>}
                        <>
                          {subItem.isYesOrNo && (
                            <select
                              name={subItem.name}
                          ...
                          {subItem.name.toLowerCase().includes('language') && (
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (!filters[item.title]?.includes(e.target.value)) {
                                  setFilters((prev: any) => {
                                    return {
                                      ...prev,
                                      [item.title]: filters[item.title]
                                        ? [...filters[item.title], e.target.value]
                                        : [e.target.value],
                                    };
                                  });
                                } else {
                    ...
                    {subItem.isDate && (
                      <div className=" flex gap-1 flex-col items-start">
                        {subItem.name.toLowerCase().includes('age') && (
                          <DateOfBirthForm filter={item.title} setFilter={setFilters} filters={filters} />
                        )}

                        {(subItem.name.toLowerCase().includes('registered') ||
                          subItem.name.toLowerCase().includes('booking')) && (
                          <DateType1 filter={item.title} setFilter={setFilters} filters={filters} />
                        )}
                      </div>
                    )}
                  </>
                        ...
                        onClick={() => {
                          setFilters((prev: any) => {
                            return {
                              ...prev,
                              [item.title]: filters[item.title] && [
                                ...filters[item.title].filter((filterItem: string) => filterItem !== im),
                              ],
                            };
                          });
                        }}
