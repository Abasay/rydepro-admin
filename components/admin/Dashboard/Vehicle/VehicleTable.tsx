...
const VehicleTable = ({
  ...
  setSelectVehicle: (vehicle: Vehicle) => void;
  deleteVehicle: (id: string) => void;
}) => {
  const getVehicle = async (id: string) => {
    try {
      const url = URLS.BASE_URL_ADMIN + URLS.getVehicle + `/${id}`;
      await GET_REQUEST(url, Cookies.get('token') || '').then((result: any) => {
        if (result.success) {
          setSelectVehicle(result.data);
          setShowModal(true);
        } else {
  ...
  return (
    <section className=" mt-6 flex flex-col gap-4 px-6 min-h-[300px] max-h-[771px]">
      <div className=" flex justify-between items-center w-full">
        <h3 className="text-[#2B2B2B]  text-2xl font-bold">Vehicle Category List</h3>

        <button
          className=" w-[195px] h-[56px] bg-[#1C2951] rounded-lg p-2 justify-center text-[#FFFFFF] text-[22px]"
              ...
              <tr className=" text-[#0E0E0E] flex items-end pb-1 pt-1 tracking-widest text-sm font-medium">
                ...
                <th className=" flex items-start gap-12 min-w-[1124px] mr-12">
                  ...
                  <th className="py-2 min-w-[140px] max-w-[140px] flex gap-2">
                    <span>Vehicle Type</span> <UpsAndDowns />
                  </th>{' '}
                  ...
                  <th className="py-2 min-w-[170px] max-w-[170px] flex gap-2">
                    <span>Capacity</span> <UpsAndDowns />
                  </th>
                  <th className="py-2 min-w-[174px] max-w-[174px] flex gap-2">
                    <span>Wait Times</span> <UpsAndDowns />
                  </th>
                  <th className="py-2 min-w-[184px] max-w-[18   4px] flex gap-2">
                    <span>Description</span> <UpsAndDowns />
                  </th>
                  <th className="py-2 min-w-[250px] max-w-[250px] flex gap-2">
                    <span>Images</span>
                    <UpsAndDowns />
                  </th>
                ...
                <th className="py-2 min-w-[124px] max-w-[300px] flex gap-2">Actions</th>
              </tr>
            </thead>
                ...
                vehicles.map((vehicle, index) => (
                  <tr key={index} className="border-b flex pt-2 hover:bg-gray-50">
                    ...
                    <td className="flex items-start gap-12 min-w-[1124px]">
                      ...
                      <td className="py-3 min-w-[140px] max-w-[140px]">{vehicle.vehicleType}</td>
                      <td className="py-3 min-w-[164px] max-w-[164px]">{vehicle.vehicleTiers}</td>
                      <td className="py-3 min-w-[140px] max-w-[140px]">{vehicle.engineType}</td>
                      <td className="py-3 min-w-[170px] max-w-[170px] flex flex-col gap-[2px]">
                        {' '}
                        <span>Passengers: {vehicle.passengerCapacity} </span>{' '}
                        <span>Luggages: {vehicle.luggageCapacity} </span>
                      </td>
                      <td className="py-3 min-w-[174px] max-w-[174px] flex flex-col gap-[2px]">
                        {' '}
                        <span>Domestic: {vehicle.waitingTimes.domestic} </span>{' '}
                        <span>International: {vehicle.waitingTimes.international} </span>{' '}
                        <span>Cruise: {vehicle.waitingTimes.cruise} </span>{' '}
                        <span>Intercity: {vehicle.waitingTimes.intercity} </span>
                      </td>
                      <td className="py-3 min-w-[184px] max-w-[184px]">{vehicle.description}</td>
                      <td className="py-3 min-w-[250px] max-w-[250px] flex gap-[2px]">
                        <Image src={vehicle.webIcon} alt="Web Icon" width={50} height={50} />
                        <Image src={vehicle.mapIcon} alt="Map Icon" width={50} height={50} className="border" />
                        <Image src={vehicle.mobileActiveIcon} alt="Mobile Active Icon" width={50} height={50} />
                        <Image src={vehicle.mobileInactiveIcon} alt="Mobile Inactive Icon" width={50} height={50} />
                        <Image
                          src={vehicle.onlineReservationIcon}
                          alt="Online Reservation Icon"
                          width={50}
                          height={50}
                        />
                      </td>
                      <td className="py-3 min-w-[138px] max-w-[138px]">
                        {new Date(vehicle.createdAt).toLocaleDateString()}
                      </td>
                      <td className="py-3 min-w-[124px] max-w-[124px]">
                        {new Date(vehicle.createdAt).toLocaleTimeString()}
                      </td>
                      <td className="py-3 min-w-[174px] max-w-[174px]">
                        {vehicle.location.latitude}° N, {vehicle.location.longitude}° W
                      </td>
                      <td className="py-3 min-w-[124px] max-w-[124px]">{vehicle.status ? 'Active' : 'Inactive'}</td>
                    </td>
                    ...
                    <td className=" flex gap-12 min-w-[300px]">
                      <td className="py-2 flex items-start gap-4 min-w-[124px] max-w-[124px]">
                        ...
                        <button className="" onClick={() => deleteVehicle(vehicle._id)}>
                          {/* <svg
                            width='20'
                            height='20'
                            viewBox='0 0 20 20'
