...
const Biometrics = () => {
  ...
  const handleCaptureImage = () => {
    ...
    async function login() {
      // const email = 'abdulsalamasheem0@gmail.com';

      // 1. Get challenge from server
      const initResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASEURL}/api/init-auth?username=${userLoginCredentials.username}`,
        {
          credentials: 'include',
        }
      );
      ...
      const verifyResponse = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/api/verify-auth`, {
        credentials: 'include',
        method: 'POST',
        headers: {
      ...
      if (verifyData.verified) {
        ...
        toast.success(`Successfully logged in ${userLoginCredentials.username}`);
      } else {
